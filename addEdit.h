...
void manage_user_role(int client_socket, int employee_id)
{
	...
	printf("Inside manage user role 527\n");
	...
	{
		printf("add edit 538 %d %d\n",temp_emp.emp_id,employee_id);
		if (temp_emp.emp_id == employee_id)
		{
			printf("manage user role 539  %d %d\n",temp_emp.is_active,employee_id);
			temp_emp.is_active = false;
			// printf("manage user role 539 %d\n"temp_emp.is_active);
