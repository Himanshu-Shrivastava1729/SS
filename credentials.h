...
{
	printf("auth customer fucntion 431 input user id:%d\n",user_id);
	...
	printf("auth customer fucntion 446 input user id:%d\n",user_id);
	while (read(fd, &temp, sizeof(struct customer)) > 0)
	{
		printf("auth customer fucntion 449 temp_cust_id:%d\n",temp.customer_id);
		if (temp.customer_id == user_id)
		{
			printf("authenticate_customer 452\n");
